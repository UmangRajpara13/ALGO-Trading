            ...
            payload.forEach(data => {
                ...
                if (data.MessageCode === 1505 & data.name === "NIFTY 50") {
                    ...
                    if (nearest_future === null
                        ...
                        for (let j = 0; j < types.length; j++) {
                            ...
                            for (let i = 0; i < 11; i++) {
                                ...
                                if (nearest_future > contractPrice) {
                                    row[`n_m_${strike_price}_${types[j]}`] = nearest_future + strike_price
                                }
                                if (nearest_future == contractPrice) {
                        ...
                        worksheet.addRow(row); // concluding row here since we only need nifty50
                    }
                }
                ...
                if (data.MessageCode === 1505 & strikeList.includes(data.name)) {
                    ...
                    if (Object.keys(row).length == 23) {
                        const candleHour = new Date(data.BarTime * 1000).getHours()
                        const candleMinute = new Date(data.BarTime * 1000).getMinutes()
                        worksheet.addRow({ ...row, time: `${candleHour}:${candleMinute}` })
                    };
                }
            });
            break;
